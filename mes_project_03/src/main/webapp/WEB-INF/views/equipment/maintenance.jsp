...
<!-- 	수정 모달 창 -->
	        ...
	        <form id="editmaintenanceForm">
				...
				<table>
			        ...
			        <tr>
			        	<th rowspan="2">설비 및 날짜</th>
			            <th>설비(코드,이름,유형으로 검색)</th>
			            <td colspan="3"><input type="text" id="equipInput" class="editInput"  placeholder="검색후 검색된 설비를 클릭하세요."><input type="hidden" name="equip_id" id="equipIdHidden">
			            	<ul id="equipList">
			            		<c:forEach var="code" items="${equipList}">
			            		<li data-equip-id="${code.equip_id}">[${code.equip_id }]${code.equip_name }(${code.type})</li>
			            		</c:forEach>
	...
	document.getElementById('submitMaintenance').addEventListener('click', function () {
        ...
        .then(data => {
            if (data.success) {
                alert('유지보수가 성공적으로 저장되었습니다.' + data.message);
                location.href = 'maintenance'; // 페이지 리다이렉트
            } else {
                alert('유지보수 저장에 실패했습니다. 다시 시도하세요.' + data.message);
            }
        })
        .catch(error => {
	...
	document.getElementById('submitEditMaintenance').addEventListener('click', function () {
        ...
        fetch('updateMaintenance.do', {
            method: 'POST',
            body: formData,
        })
        .then(response => {
		    console.log(response); // 응답 객체를 확인
 	...
 	////////////////////////////////////////////////////////////////////////////////////////
 	// 각 삭제 버튼에 이벤트 리스너 추가
 	const deleteMaintenanceBtn = document.querySelectorAll('.deleteMaintenanceBtn'); // 설비 삭제 버튼
	deleteMaintenanceBtn.forEach((button) => {
	    button.addEventListener('click', (event) => {
	    	event.preventDefault();
	        const row = event.target.closest('tr'); // 클릭된 버튼의 부모 행 가져오기
	        const main_id = row.cells[0].textContent; // main_id 가져오기

	        // 삭제 확인 창 표시
	        const confirmed = confirm(`유지보수 ID: \${main_id}를 정말로 삭제하시겠습니까?`);

	        if (confirmed) {
	            // 삭제 요청 전송
	            fetch(`deleteMaintenance.do?main_id=\${main_id}`, {
