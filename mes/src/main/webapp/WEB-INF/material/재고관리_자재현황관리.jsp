...
<body>
	...
	<div id="addRowModal" class="modal">
		<div class="modal-content">
			...
			<h2>추가할 자재 정보 입력</h2>
			<form id="addRowForm">
				<label for="prodNum">자재번호:</label> <input type="text" id="prodNum"
					name="prodnum" required><br>
				<label for="LOTNum">LOT번호:</label> <input type="text" id="lotnum"
					name="lotnum" required><br>
				<label for="prodName">자재이름:</label> <input type="text" id="prodName"
					name="prodname" required><br>
				<label for="count">규격:</label> <input type="text" id="spec"
					name="spec" required><br>
				<label for="loc">저장위치:</label> <input type="text" id="loc"
					name="loc" required><br>
	...
	<div class="tableID">
		...
		<table id="dataTable">
			...
			<tbody>
				<% List<MaterialStatusDTO> list = (List<MaterialStatusDTO>)
						request.getAttribute("list");


						// 데이터 삽입
						for (int j = 0; j < list.size(); j++) { String
							prodnum=list.get(j).getProdnum(); String
							lotnum=list.get(j).getLotnum(); String
							prodname=list.get(j).getProdname(); int
							tquantity=list.get(j).getTquantity(); String
							spec=list.get(j).getSpec(); String
							storage_loc=list.get(j).getStorage_loc(); Date
							date=list.get(j).getEditdate(); %>
							<tr>
								...
								<td><button
										onclick='editRow("<%=prodnum%>")'>수정</button>
								</td>
							</tr>
							<% } %>
			</tbody>
